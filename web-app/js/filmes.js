function filmeController($scope, $http){

    var path = "/educafro/filme/";
    $scope.temaAtual = "";
    $scope.mostrar = false;
    $scope.listagem = function(){
        $http.get(path+"listagem").success(function(resposta){
            $scope.filmes = resposta;
        });
    };
    ...
    $scope.editar = function(id, tema){
        ...
        $http.get(path+"buscarPorId/"+id).success(function(resposta){
            $scope.filme = angular.copy(resposta[0]);
            $scope.filme.dataInclusao = $scope.formatarData(resposta[0]["dataInclusao"]);
