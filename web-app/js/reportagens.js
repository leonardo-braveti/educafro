function reportagemController($scope, $http){

    var path = "/educafro/reportagem/";
    $scope.temaAtual = "";
    $scope.mostrar = false;
    $scope.listagem = function(){
        $http.get(path+"listagem").success(function(resposta){
            $scope.reportagens = resposta;
        });
    };
    ...
    $scope.editar = function(id, tema){
        ...
        $http.get(path+"buscarPorId/"+id).success(function(resposta){
            $scope.reportagem = angular.copy(resposta[0]);
            $scope.reportagem.dataInclusao = $scope.formatarData(resposta[0]["dataInclusao"]);
